...
export default function RecordCollectionModal({
  ...
  useEffect(() => {
    if (!isOpen) return;
    db.categories.where("type").equals("income").toArray().then((cats) => {
      db.categories.toArray().then((all) => {
        const income = all.filter((c) => !c.type || c.type === "income");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await db.transactions.add({
        type: "income",
        amount: paid,
        description: `Collection from ${debt.personName}${debt.description ? ` – ${debt.description}` : ""}`,
      ...
      window.dispatchEvent(new CustomEvent("show_toast", {
        detail: {
          ...
          description: `Successfully recorded ₱${paid.toLocaleString("en-US", { minimumFractionDigits: 2 })} from ${debt.personName}.`,
          type: "success"
        }
      }));
    } catch (error) {
      ...
      >
        ...
        <div className="flex justify-between items-start">
          <div className="flex flex-col gap-1.5 max-w-[90%]">
            ...
            <p className="text-gray-500 text-sm leading-relaxed mt-1">
              From <span className="font-semibold text-gray-700">{debt.personName}</span> · Remaining:{" "}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium tracking-wide text-gray-500">Amount received</label>
            <input
              type="number"
              min="0.01"
              step="0.01"
          ...
          {/* Account Selector Button (matches IncomeModal style) */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium tracking-wide text-gray-500">Deposit to account</label>
            <button
              type="button"
          ...
          {/* Category selector */}
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium tracking-wide text-gray-500">Income category <span className="font-normal text-gray-400 text-xs">(optional)</span></label>
          ...
          >
            ...
            <div className="flex justify-between items-center">
              <div className="flex flex-col gap-0.5">
                ...
                <p className="text-xs text-gray-400">Choose destination account for collection.</p>
              </div>
              <button
                onClick={() => {
                  setIsAccountSelectorOpen(false);
