...
export default function IncomeModal({ isOpen, onClose }: IncomeModalProps) {
  ...
  const categories = useLiveQuery(async () => {
    ...
    return list.filter((c) => c.type === "income");
  }) || [];
  ...
  const selectedAccount = accounts.find(a => a.id?.toString() === toAccountId);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await db.transaction("rw", [db.accounts, db.transactions], async () => {
        const updatedToBalance = toAcc.type === "Credit"
      ...
      window.dispatchEvent(new CustomEvent("show_toast", { 
        detail: { 
          title: "Income Logged", 
          description: `₱${amount} to ${toAcc.name}`, 
          type: "success" 
        } 
      }));
      // Reset
      ...
      >
        ...
        <div className="flex justify-between items-start">
          <div className="flex flex-col gap-1.5 max-w-[90%]">
            <h2 className="text-2xl font-bold text-gray-900 leading-none">Add income</h2>
            <p className="text-gray-500 text-sm leading-relaxed">
              Record incoming money into the imported session and refresh the dashboard immediately.
            </p>
          </div>
          <button 
            onClick={onClose}
        ...
        {/* Form Fields - 2 column grid matching AddAccountModal */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-gray-400 font-medium">Saved in {currencyLabel}.</span>
            </div>
            <input 
              type="text" 
              value={amount}
          ...
          <div className="flex flex-col gap-2">
            ...
            <input 
              ...
              className="w-full bg-white border border-gray-100 rounded-xl px-4 py-3 text-base text-gray-950 font-medium placeholder-gray-400 focus:outline-none focus:border-purple-500/50 shadow-sm"
              placeholder="Salary, side gig, refund..."
            />
          </div>
          ...
          >
            ...
            <div className="flex justify-between items-center">
              <div className="flex flex-col gap-0.5">
                ...
                <p className="text-xs text-gray-400">Choose destination account for income.</p>
              </div>
              <button
                onClick={() => {
                  setIsAccountSelectorOpen(false);
